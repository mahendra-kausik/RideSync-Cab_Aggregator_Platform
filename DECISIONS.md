...
## P-006 — Live production incident: every `/api/*` route hung indefinitely (Redis stale-connection + unbounded wait)
- **Date / Layer:** 2026-07-24 / discovered while verifying P-002's Atlas seeding
- **Context:** While testing demo-account logins against the deployed Render URL, requests to
  `/api/auth/login-phone` never returned (25s+, no response), while `/health` responded in ~300ms. Narrowed
  it down: `/api` itself (a static, DB/Redis-free handler) also hung, so the failure was in shared `/api/*`
  middleware, not any one route's logic. `apiRateLimiter` (mounted globally on `/api`) calls
  `redisClient.call(...)` directly via `rate-limit-redis`'s `sendCommand`, with no timeout. Combined with
  `maxRetriesPerRequest: null` (the P-005 fix), a command sent on a connection that *looks* open but is
  actually stale (Upstash silently drops idle TCP connections without a clean FIN/RST the client would
  notice) waits forever instead of erroring or reconnecting. `sessionManager.createSession` has the same
  unguarded pattern, so login would have hung even without the rate limiter. Verified Upstash itself was
  healthy the whole time — a fresh `ioredis` connection from a local machine `PING`'d successfully in
  ~1.2s — so this was specifically Render's long-lived connection going stale, not a Redis outage.
- **Action:** Added `commandTimeout: 5000` to the single shared `ioredis` client in `backend/config/redis.js`
  — bounds every Redis command (rate limiter, sessions, socket adapter) to 5s. A hung command now rejects
  instead of waiting forever; since every route is wrapped in `asyncHandler`, the rejection becomes a normal
  error response through `globalErrorHandler` rather than a crash or a hang. `maxRetriesPerRequest: null`
  is unchanged — that governs reconnect retries on a known-broken connection, not per-command wait time on a
  connection that appears healthy, so this doesn't reintroduce the P-005 crash.
- **Why:** Root-cause fix at the one shared client, not a patch on each of the ~3+ call sites (rate limiter,
  session manager, future Redis consumers) that would otherwise each need their own timeout guard.
- **Tradeoffs / risks:** A stale connection now surfaces as a real (bounded) error to the end user for up to
  5s during the window before `ioredis` detects and reconnects, rather than being invisible — acceptable;
  bounded and recoverable beats unbounded and silent. 5s chosen to comfortably exceed Upstash's observed
  ~1.2s round-trip while still failing well within a typical request timeout.
- **Verification:** `npm run lint` 0/0, `npm test` 164/164. Deployed via the existing Render auto-deploy
  hook; re-tested `/api/auth/login-phone` and `/api` against the live URL post-deploy (see PROGRESS.md).
